...
parser.add_argument('--down-ratio', default=2, type=int, help='stitching downscale ratio')
...
def DrawGrid(img, coord, shape, thickness=2, color=(255,255,255,255)):
    cv2.rectangle(img, tuple(np.maximum([0, 0], coord-thickness//2)), tuple(coord - thickness//2 + np.array(shape)), (255, 255, 255, 255), thickness=thickness)
    return img
...
def StitchPatches(hdf5_file_path, pred_path,downscale=4, draw_grid=False, bg_color=(0,0,0), alpha=-1):
    ...
    images = ones((len(files), 256, 256, 3))
...
stitch_path = os.path.join(out_path, "fake_stitch_last"+'.png')
